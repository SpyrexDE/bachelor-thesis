...
The agent-system design follows Dibia, *Designing Multi-Agent Systems: Principles,
Patterns, and Implementation for AI Agents* (self-published, 2025; companion code
`picoagents`) for its code decomposition — agent = model + instructions + interface,
typed messages, topologies as pure wiring, termination conditions (decisions.md D12).
The mapping:
...
| `topologies/` — wiring only; roles are identical everywhere | Workflow patterns (Dibia's explicit-control side): a parallel fan-out and a conditional revise-loop — never the dynamic/autonomous patterns |
...
a framework would blur both (decisions.md D12). The topologies are static workflow
wiring by design: the executor walks a fixed graph, never an agent composing it at
runtime. Even Fine's critic loop is not autonomous control — the critic's
accept/revise verdict is the one model-driven branch, but the loop and its
termination are predefined code, pinned in `stopping.py` and tests.
...
Two seams exist because the deployment targets (Henkel, Docker) will
...
over the trace's call graph, so parallel producer calls count once. The trace
timeline is virtual: a call starts when its parents have finished, and per-call
durations place it on that timeline (`harness/trace.py`). Real providers report
measured durations; the mock reports simulated ones with plausible magnitudes
(decisions.md D4).

## Deployment

- **Dev**: `docker compose -f docker-compose.yml -f docker-compose.dev.yml up` — source
  mounted, hot reload; the full test suite runs in the same image via
...
- **Henkel** (later): same image contract — configuration only via
  `GRAIN_*` environment variables, `/health` endpoint, single exposed port, no
  external assets at runtime. Swapping mock for Azure and SQLite for Henkel's
...
`GRAIN_ADMIN_TOKEN` (optional; when set, researcher endpoints require it, so
raters sharing the host cannot open the researcher console and unblind
themselves — set it before sending raters a session link, D13; unset is fine for
local work). Port and host belong to the uvicorn command line / Dockerfile. No
config files.
