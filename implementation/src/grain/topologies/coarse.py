"""Coarse: orchestrator-workers, restricted (concept/01): the orchestrator
emits only the shared concept, then producers work in parallel with no
contact. Nothing synthesises the outputs."""

from grain.agents.messages import SharedConcept
from grain.agents.roles import OrchestratorAgent
...
def emit_concept(execution: Execution) -> tuple[SharedConcept, int]:
    seed = call_seed(execution.run_seed, "orchestrator:0")
    concept, response = OrchestratorAgent(execution.provider).emit_concept(execution.brief, seed)
    idx = record_chat(
        execution, response, role="orchestrator", agent="orchestrator",
...
def initial_production(execution: Execution) -> tuple[SharedConcept, dict[str, Draft]]:
    concept, orchestrator_idx = emit_concept(execution)
    drafts = {
        spec.id: produce_artifact(execution, spec, concept=concept, parents=(orchestrator_idx,))
